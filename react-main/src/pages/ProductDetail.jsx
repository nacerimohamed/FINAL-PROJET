    ...
    <>
      ...
      <div className="min-h-[60vh] flex items-center justify-center" dir={isRTL ? 'rtl' : 'ltr'}>
        <div className="text-center">
          ...
          <p className="text-gray-400 text-sm">{t('productDetail.loading')}</p>
        </div>
      </div>
      <Footer />
    </>
    ...
    <div className="bg-gray-50 min-h-screen flex flex-col" dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      <div className="bg-white border-b border-gray-100">
        <div className="max-w-6xl mx-auto px-4 py-2.5">
          <div className="flex items-center gap-2 text-xs text-gray-500" style={{ flexDirection: isRTL ? 'row-reverse' : 'row' }}>
            <BackButton className="!mb-0 !p-1.5 !bg-transparent !hover:bg-gray-100" />
            <Link to="/" className="hover:text-gray-700">{t('productDetail.home')}</Link>
            <span>/</span>
            <Link to="/products" className="hover:text-gray-700">{t('productDetail.products')}</Link>
            <span>/</span>
      ...
      <div className="flex-1 max-w-6xl mx-auto px-4 py-6 w-full">
        <div className={`grid md:grid-cols-2 gap-8 items-start ${isRTL ? 'lg:flex-row-reverse' : ''}`}>
            ...
            <div className="bg-white rounded-lg overflow-hidden border border-gray-200">
              <div className="aspect-[4/3] relative">
                ...
                {(product.category || product.categorie) && (
                  <div className={`absolute top-3 ${isRTL ? 'right-3' : 'left-3'} bg-white/90 backdrop-blur-sm text-gray-700 px-2 py-1 rounded-md text-xs font-medium shadow-sm`}>
                    {product.category || product.categorie}
                  </div>
                )}
              </div>
          ...
          <div className="space-y-4">
            ...
            <Link 
              ...
              className={`inline-flex items-center gap-1.5 text-xs font-medium text-emerald-600 hover:text-emerald-700 bg-emerald-50 px-2.5 py-1 rounded-md w-fit ${isRTL ? 'flex-row-reverse' : ''}`}
            >
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-4 space-y-4 mt-4">
              ...
              <div className={`flex items-center justify-between pt-2 border-t border-gray-100 ${isRTL ? 'flex-row-reverse' : ''}`}>
                <span className="text-sm font-medium text-gray-700">{t('productDetail.total')}</span>
              ...
              >
                ...
                <FaArrowRight className={`text-sm group-hover:translate-x-1 transition-transform ${isRTL ? 'rotate-180' : ''}`} />
              </button>
            </div>
          </div>
        </div>
      ...
      {/* Purchase Modal */}
      {showPurchaseModal && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4" dir={isRTL ? 'rtl' : 'ltr'}>
